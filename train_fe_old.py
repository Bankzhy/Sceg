...
def test():
    ...
    for batched_graph, batched_neg_graph, labels in test_data_loader:
        ...
        node_features = {'class': class_feats, 'method': method_feats}
        pos_score, neg_score = model(batched_graph, batched_neg_graph, node_features, ('class', 'include', 'method'))
        # 假设 pos_score 是正类的预测分数
        probs = torch.sigmoid(pos_score).detach().cpu().numpy()  # 如果输出是logits
        preds = (probs > 0.5).astype(int)  # 转成0/1标签
