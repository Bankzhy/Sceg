...
def mark_pos_nodes():
    ...
    query = """select * from lc_master where label=1 and graph not like '%"is_extract": 1%';"""
        ...
        try:
            ...
            if lc_name.startswith("ihc") or lc_name.startswith("ilc"):
                ...
                for index, node in enumerate(nodes):
                    ...
                    if merge_method_nodes[index-1]["name"] in extract_methods:
...
def check_nodes():
    cursor = db.cursor()
    query = "select * from lc_master where label=1 and `group`='m' and extract_methods != '';"
    cursor.execute(query)
    failed = []
    rows = cursor.fetchall()
    print("total: ", len(rows))
