...
def fetch_block_lines(block):
    start=10000
    end=0
    for statement in block:
        if statement.start_line<start:
            start=statement.start_line
        if statement.end_line>end:
            end=statement.end_line
    return (start,end)
    ...
    for program in sr_project.program_list:
        for sr_class in program.class_list:
            for sr_method in sr_class.method_list:
                ...
                print(candidates_block_list)
                for block in candidates_block_list:
                    if len(block) > 0:
                        block_lines = fetch_block_lines(block)
                        block_list.append(block_lines)
    return block_list

def parse_statement_list(statement_list):
    candidates_list = []
    new_group = []
    for statement in statement_list:
        ...
        if type(statement) is SRIFStatement:
            pos_child_statement_list = parse_statement_list(statement.pos_statement_list)
            candidates_list.extend(pos_child_statement_list)
            neg_child_statement_list = parse_statement_list(statement.neg_statement_list)
            candidates_list.extend(neg_child_statement_list)
        elif type(statement) is SRFORStatement:
            child_statement_list = parse_statement_list(statement.child_statement_list)
            candidates_list.extend(child_statement_list)
        elif type(statement) is SRWhileStatement:
            child_statement_list = parse_statement_list(statement.child_statement_list)
            candidates_list.extend(child_statement_list)
        elif type(statement) is SRTRYStatement:
            try_statement_list = parse_statement_list(statement.try_statement_list)
...
def adjust_predictions(list_a, list_b):
    list_a = list_a[:]  # make a copy to avoid modifying the original
    for start, end in list_b:
        sublist = list_a[start:end+1]  # slice inclusive
        if sum(sublist) > len(sublist) / 2:  # more than 50% are 1
            for i in range(start, end+1):
                if start>=0 and end<len(list_a):
                    list_a[i] = 1
    return list_a
...
def eval_model_refact():
    ...
    model_path = r"output/model/lmr-model-gat.pkl"
    cursor = db.cursor()
        ...
        for index, row in enumerate(reader):
            ...
            if lm_row is not None:
                ...
                if lm_label == 1:
                    lm_graph = lm_row[8]
                    lm_content = lm_row[2]
                    block_list = fetch_all_blocks(lm_content)
                    ...
                    prediction = adjust_predictions(prediction, block_list)
