...
def fetch_nodes(code):
    ...
    if "public " in code:
        code = code.replace("public ", "public void ")

    if "private " in code:
        code = code.replace("private ", "private void ")
...
def mark_pos_nodes():
    ...
    cursor.execute("""select * from lm_master where label=1 and graph not like '%"is_extract":%';""")
